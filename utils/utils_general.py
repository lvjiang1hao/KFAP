...
class Lang:  # 建立一个有word2index和index2word的词表
    def __init__(self):
        self.word2index = {}
    ...
    def index_words(self, story, trg=False):        # 处理三元组形式
        if trg:
    ...
    def index_word(self, word):        # 获得词的index
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        context_arr = self.preprocess(context_arr, self.src_word2id, trg=False)         # 转换为id形式,有三元组
        response = self.data_info['response'][index]
        response = self.preprocess(response, self.trg_word2id)          # 无三元组
        ...
        conv_arr = self.preprocess(conv_arr, self.src_word2id, trg=False)           # 有三元组
        kb_arr = self.data_info['kb_arr'][index]
        kb_arr = self.preprocess(kb_arr, self.src_word2id, trg=False)       # 有三元组
        sketch_response = self.data_info['sketch_response'][index]
        sketch_response = self.preprocess(sketch_response, self.trg_word2id)         # 无三元组
        ...
        data_info['context_arr_plain'] = self.data_info['context_arr'][index]           # plain为原始的单词形式
...
def get_seq(pairs, lang, batch_size, type):
    data_info = {}
    for k in pairs[0].keys():           # 得到如{'context_arr': [], 'response': [], 'sketch_response': []}
        data_info[k] = []
