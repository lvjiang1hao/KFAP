...
def read_langs(file_name, max_line=None):
    ...
    with open('data/CamRest/camrest676-entities.json') as f:
        global_entity = json.load(f)  # global_entity包含数据集中的所有实体，是一个dict，包含event,time,date,party,room,agenda,location,weekly_time,temperature,weather_attribute,traffic_info,poi_type,poi,distance
        ...
        for line in fin:
            ...
            if line:
                ...
                if '\t' in line:  # 若有\t在句子中，则这句话为输入，回复，真实实体
                    ...
                    # ent_index = list(set(ent_idx_cal + ent_idx_nav + ent_idx_wet))  # set中无重复的元素，此操作目的为去掉重复的元素
                    ent_index = list(set(ent_idx))
